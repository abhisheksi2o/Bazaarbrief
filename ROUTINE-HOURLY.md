...
STEP 2 - read_db, db_op "get", collection "feed", doc_id "latest", with out_dir "./prev" (this saves the current feed to ./prev/feed/latest.json and reports its version number; note the version). Do not read its contents into the conversation. Also read_db "get" markets/latest with the same out_dir and note its version.
...
STEP 5 - Publish with write_db, db_op "set", using file_path, one call per document, and pin each write with if_version set to the version number the read in STEP 2 reported: feed/latest from out/feed.json, markets/latest from out/markets.json. If a write is rejected with version_mismatch, read that document again with read_db (out_dir "./prev2") to get its current version and resend the same write with that if_version. Never delete documents.
